...
/**
 * Upload a generic media buffer (image or video) to Cloudinary.
 */
export const uploadMedia = (buffer: Buffer, filename: string): Promise<{ url: string; type: string }> => {
  return new Promise((resolve, reject) => {
    const isVideo = filename.match(/\.(mp4|mov|avi|wmv|mkv)$/i);
    const resourceType = isVideo ? 'video' : 'image';
    
    const stream = cloudinary.uploader.upload_stream(
      {
        folder: 'buddystore/broadcasts',
        public_id: filename,
